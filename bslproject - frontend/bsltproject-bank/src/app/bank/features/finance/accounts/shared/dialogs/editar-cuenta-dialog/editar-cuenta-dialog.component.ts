...
@Component({
  ...
  styleUrls: ['./editar-cuenta-dialog.component.css']
})
export class EditarCuentaDialogComponent implements OnInit, OnDestroy {
  form!: FormGroup;
  cuenta!: Cuenta;
  ...
  ngOnInit(): void {
    ...
    const roles = Array.isArray(user.roles) ? user.roles.map((r: string | Rol) => typeof r === 'string' ? r : (r as Rol).nombre) : [];
    this.esAdmin = roles.includes('ADMIN');

    this.inicializarFormulario();

    this.tipoSub = this.form.get('tipo_cuenta')!.valueChanges.subscribe(tipo => {
