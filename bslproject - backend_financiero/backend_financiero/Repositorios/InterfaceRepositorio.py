...
class InterfaceRepositorio(Generic[T]):
    ...
    def loadFileConfig(self):
        ...
        config_path = os.path.join(base_dir, "../Config/config.json")  # Ajusta la ruta correctamente
    ...
    def findById(self, id):
        ...
        if x is None:  # ✅ Si no se encuentra, retornamos None antes de procesar
            return None

        x = self.getValuesDBRef(x)  # ✅ Solo ejecutamos esto si `x` no es None
        x["_id"] = x["_id"].__str__()
        return x
